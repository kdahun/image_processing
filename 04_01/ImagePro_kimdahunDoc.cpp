...
// ImagePro_kimdahunDoc.cpp: CImageProkimdahunDoc 클래스의 구현
//
...
{
	if (ar.IsStoring())//carchive 클래스 안에 isstoring이 있다
	{
	}
	else
	{
...
void CImageProkimdahunDoc::Pixeltimes()
{
	// TODO: 여기에 구현 코드 추가.
	int value;
	for (int y = 0; y < 256; y++) {
		for (int x = 0; x < 256; x++) {
			value = inputImg[y][x] * 1.2;
			if (value > 255)resultImg1[y][x] = 255;
			else resultImg1[y][x] = value;
		}
	}
}
...
void CImageProkimdahunDoc::PixelHistoEq()
{
	...
	for (y = 0; y < 256; y++) {
		for (x = 0; x < 256; x++) {
			k = inputImg[y][x];
			resultImg2[y][x] = (unsigned char)(sum[k] / N * 255);
		}
	}
}
...
void CImageProkimdahunDoc::LoadImageFile(CArchive& ar)
{
	...
	if (strcmp(strrchr(fname, '.'), ".ppm") == 0 ||
		...
		if (fp->GetLength() != 256 * 256) {
			AfxMessageBox(CString("256*256크기의 파일만 사용가능합니다."));
			return;
		}
		imageWidth = 256;
		imageHeight = 256;
